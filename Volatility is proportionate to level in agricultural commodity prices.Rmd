...
plotRiskReward <- function(dfPlot, labelItems, thisTitle, m, b, N, adjR2){
...
ggRiskRewardExpPrice <- plotRiskReward(dfRiskReward, labelItems, thisTitle, m, b, N, adjR2)
#============================================================================
# Farmgate price series
#============================================================================
thisFile <- "Production_Crops_Livestock_E_All_Data.csv"
thisFilepath <- paste0(thisFolder, thisFile)
theseElements <- c("Production", "Area harvested"); theseUnits <- c("t", "ha")
dfAreaProd <- processDf(thisFilepath, theseElements, theseUnits, isExpPrice = F) %>%
  spread(Element, val) %>% select(Item, year, Production, `Area harvested`)
...
ggRiskRewardProdKgPrice <- plotRiskReward(dfRiskReward, labelItems, thisTitle, m, b, N, adjR2)
dfHa <- df %>% rename(priceSeries = haPrice)
dfRiskReward <- processRiskRewardDf(dfHa)
dfRiskReward <- dfRiskReward %>% subset(group != "Other")
thisTitle <- "World producer real price (2014-16 USD/ha)\nMean and standard deviation over years 1961-2024"
ggRiskRewardProdHaPrice <- plotRiskReward(dfRiskReward, labelItems, thisTitle, m, b, N, adjR2)

```

the analysis is meaningful insofar as these time series are stationary
...
**Fig 11: The higher the price, the more volatile it is.** 'Export price' = Export Value/Export Quantity.

'Farm gate price' equals the FAO Value of Agricultural Production (VAP) series divided by the Production series. 'Revenue yield' equals the FAO VAP series divided by the Area harvested series. The results are converted to constant 2024 dollars using deflation factors provided by Sahr (2021)

&nbsp;

The relation between price level and volatility also appears to persist over a wide range of food price epochs, from the oil crisis of 1973 through to the more recent volatile period beginning in 2007. One notices this also appears to be true for farmgate prices and revenue yields (right panel, Fig \ref{fig:priceSeries}). A plot of the logged means of these series against their respective logged standard deviations in Fig \ref{fig:riskReward} confirms a tight correspondence between expected value and volatility. (See S2 and S3 Tables for details on the commodity grouping in this plot.)
